terraform {
  backend "s3" {
    ...
    key            = "staging-analytics-api/terraform.tfstate"
    region         = "us-east-1"
    profile        = "adiyaolu"
  }
}
...
locals {
  ...
  base_name = "staging-adiyaolu-analytics-api"
  
  # base_name_short is similar to base_name, There are some resource types which have a 32 char limit
  base_name_short = "stage-analytics"
  
  # The prefix for the description field, used to identify which environment a resource belongs to
  base_description = "Staging adiyaolu Analytics API"
 
  # sub-domain of the  Annalytcis API
  api_domain = "staging-analytics-api-performance.adiyaolu.io"
   
  # list of web ulrs which are white listed for CORS 
  api_cors_allow_origin = "https://staging-performance.adiyaolu.io, https://staging-canvas-performance.adiyaolu.io, http://localhost:3000, http://localhost:3002"
...
locals {
  ...
  docker_repository_name = "staging-analytics-api"
  docker_image = "859456250655.dkr.ecr.ca-central-1.amazonaws.com/${local.docker_repository_name}:latest"
