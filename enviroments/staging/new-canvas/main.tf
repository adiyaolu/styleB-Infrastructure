terraform {
  backend "s3" {
    ...
    key            = "staging-new-canvas-adiyaolu/terraform.tfstate"
    region         = "us-east-1"
    profile        = "adiyaolu"
  }
}
...
locals {
  base_name = "staging-performance-canvas"
  region = "us-east-1"
 profile = "testinground"
  base_description = "Staging Performance New Canvas"
  site_domain = "staging-canvas-performance.adiyaolu.io"
