terraform {
  backend "s3" {
    ...
    key     = "qa2-analytics-api/terraform.tfstate"
    region  = "us-west-1"
...
locals {
  ...
  docker_repository_name = "qa2-analytics-api"
  docker_image           = "859456250655.dkr.ecr.us-west-1.amazonaws.com/${local.docker_repository_name}:latest"
