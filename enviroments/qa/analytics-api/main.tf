terraform {
  backend "s3" {
    ...
    key            = "qa-analytics-api/terraform.tfstate"
    region         = "us-east-1"
    profile        = "adiyaolu"
  }
}
...
locals {
  base_name = "qa-adiyaolu-analytics-api"
  base_name_short = "stage-analytics"  
  region = "ca-central-1"
 profile = "testinground"
  base_description = "QA adiyaolu Analytics API"
  
  site_zone = "adiyaolu.io."
  secrets_base = "adiyaolu.staging"

  api_domain = "qa-analytics-api-performance.adiyaolu.io"
...
module "qa-docker-api" {
  ...
  docker_image = "859456250655.dkr.ecr.ca-central-1.amazonaws.com/staging-analytics-api:latest"
...
module "container_repository" {
  source = "../../../modules/ecr"
  repository_name = "staging-analytics-api"
}